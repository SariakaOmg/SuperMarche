...
<body>
  ...
  <header class="topbar">
    ...
    <div class="topbar-right">
      ...
      <span class="caisse-badge"> <?= esc(session()->get('caisse_choisie')) ?></span>
  ...
  <div class="layout">
    ...
    <div style="display:flex; flex-direction:column; gap:1.25rem;">
      ...
      <div class="card">
        ...
        <!-- En CI : action="<?= base_url('achat/ajouter') ?>" method="POST" -->
        <form id="addForm" class="add-form" action="#" method="POST">
          <!-- <?= csrf_field() ?> -->

          <div class="form-group">
            <label for="produit_id">Produit</label>
            <!-- En CI : populated via foreach($produits as $p) -->
            <select id="produit_id" name="produit_id">
              <option value="">-- Sélectionner --</option>
              <option value="1" data-prix="1500">Riz (1kg) — 1 500 Ar</option>
              <option value="2" data-prix="800">Huile (0.5L) — 800 Ar</option>
              <option value="3" data-prix="500">Pain — 500 Ar</option>
              <option value="4" data-prix="2200">Sucre (1kg) — 2 200 Ar</option>
              <option value="5" data-prix="3500">Lait (1L) — 3 500 Ar</option>
            </select>
          </div>

          <div class="form-group">
            <label for="quantite">Quantité</label>
            <input type="number" id="quantite" name="quantite" value="1" min="1" style="width:90px;" />
          </div>
      ...
      <div class="card">
        ...
        <div class="table-wrapper">
          <!-- En CI, ce tableau serait généré par foreach($lignes as $l) -->
          <table>
            <thead>
              <tr>
                <th>#</th>
            ...
            <tbody id="lignesBody">
              <!-- Lignes injectées par JS (ou CI en production) -->
              <tr class="empty-row">
                <td colspan="6">
      ...
      <div class="card recap">
        ...
        <div>
          ...
          <div class="info-row">
            ...
            <span class="val">caissier01</span>
          </div>
          <div class="info-row">
        ...
        <div class="actions">
          <!-- En CI : action="<?= base_url('achat/cloturer') ?>" -->
          <button class="btn btn-success" id="btnCloturer" onclick="cloturerAchat()">
            ✓ Clôturer l'achat
          </button>
          <!-- En CI : action="<?= base_url('caisse/changer') ?>" -->
          <a href="choix_caisse.html" class="btn btn-outline" style="text-align:center; text-decoration:none;">
            Changer de caisse
          </a>
        </div>
      </div>
  ...
  <script>
    // ── État local (en production, données viennent du serveur CI) ──
    var lignes = [];
    var compteur = 0;

    var produits = {
      '1': { nom: 'Riz (1kg)', prix: 1500 },
      '2': { nom: 'Huile (0.5L)', prix: 800 },
      '3': { nom: 'Pain', prix: 500 },
      '4': { nom: 'Sucre (1kg)', prix: 2200 },
      '5': { nom: 'Lait (1L)', prix: 3500 }
    };
    ...
    function renderTable() {
      ...
      if (lignes.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">'
          ...
          + '<circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>'
          + '<path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>'
          + '</svg>Aucun article ajouté.</div></td></tr>';
        updateRecap();
        return;
      }
    ...
    function updateRecap() {
      ...
      var nbArticles = lignes.reduce(function (s, l) { return s + l.quantite; }, 0);
    ...
    function supprimerLigne(id) {
      ...
      renderTable();
      // En CI : envoyer une requête POST à achat/supprimer/{id}
    }

    function cloturerAchat() {
      if (lignes.length === 0) {
        alert('Aucun article dans l\'achat.');
        return;
      }
      if (confirm('Clôturer cet achat ? La liste sera vidée pour le prochain client.')) {
        lignes = [];
        compteur = 0;
        renderTable();
        alert('Achat clôturé avec succès !');
        // En CI : soumettre un POST à achat/cloturer
      }
    }

    // Ajout d'un produit (côté client pour la démo)
    document.getElementById('addForm').addEventListener('submit', function (e) {
      ...
      var p = produits[produitId];

      // Regrouper si même produit déjà dans la liste
      ...
      // En CI : soumettre le formulaire normalement vers achat/ajouter
    });
  </script>
</body>
</html>
