          ...
          <div class="form-group">
            ...
            <select id="produit_id" name="produit_id">
              ...
              <?php foreach ($produits as $produit) { ?>
                <option value="<?= esc($produit['id']) ?>" data-prix="<?= esc($produit['prix_unitaire']) ?>">
                  <?= esc($produit['libelle']) ?> — <?= esc(number_format($produit['prix_unitaire'], 0, ',', ' ')) ?> Ar
                </option>
              <?php } ?>
            </select>
          </div>
...
<script>
    // ── Données injectées depuis CodeIgniter ──
    var produits = {};
    <?php foreach ($produits as $p): ?>
    produits['<?= $p['id'] ?>'] = {
        id: <?= $p['id'] ?>,
        nom: "<?= addslashes($p['libelle']) ?>",
    ...
    function renderTable() {
        ...
        if (lignes.length === 0) {
            tbody.innerHTML =
                '<tr><td colspan="6"><div class="empty-state">Aucun article ajouté.</div></td></tr>';
            updateRecap();
            return;
        }
    ...
    function updateRecap() {
        var total = lignes.reduce(function (s, l) {
            return s + l.prix * l.quantite;
        }, 0);

        var nbArticles = lignes.reduce(function (s, l) {
            return s + l.quantite;
        }, 0);
    ...
    document.getElementById('addForm').addEventListener('submit', function (e) {
        ...
        // Regrouper si déjà présent
        var existant = lignes.find(function (l) {
            return l.produitId == produitId;
        });
